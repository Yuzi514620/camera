...
$current_category_id = isset($_GET['category_id']) ? intval($_GET['category_id']) : null;
$is_deleted = isset($_GET['is_deleted']) ? intval($_GET['is_deleted']) : null;
...
// 建立 WHERE 條件
$where = "WHERE 1=1 AND article.is_deleted = 0";
$params = [];

if ($search !== '') {
  $where .= " AND (article.title LIKE :search_title OR article.content LIKE :search_content)";
...
if ($is_deleted === 1) {
  $where = "WHERE article.is_deleted = 1";
}

// 獲取分類列表
$categories = [];
try {
  $cat_stmt = $pdo->prepare("SELECT id, name FROM article_category");
  $cat_stmt->execute();
...
$archived_count = 0;
try {
  $archive_stmt = $pdo->prepare("SELECT COUNT(*) FROM article WHERE is_deleted = 1");
  $archive_stmt->execute();
  $archived_count = $archive_stmt->fetchColumn();
} catch (PDOException $e) {
  echo "錯誤: " . $e->getMessage();
  exit();
}

if ($search !== '' || $current_category_id !== null || $is_deleted === 1) {
  // 搜尋或分類過濾或下架文章顯示
          ...
          <div class="d-flex justify-content-between align-items-center pe-5 ps-1">
            <div class="input-group" style="width: 20%;">
              <form class="d-flex" method="GET" action="article.php">
                <input type="search" class="form-control border border-secondary rounded-end-0 form-control-sm " placeholder="搜尋文章" name="search" value="<?= $_GET["search"] ?? "" ?>" style="height: 38px; border-radius:10px 0 0 10px;">
            ...
            <div class="btn-group">
              ...
              <a href="?is_deleted=1" class="btn btn-secondary btn-color <?= isset($_GET['is_deleted']) ? 'active' : '' ?>">
                下架文章 <span class="badge bg-white text-danger border rounded-circle"><?= $archived_count ?></span>
                        ...
                        <?php if ($search !== '' || $current_category_id !== null || $is_deleted === 1): ?>
                          <?php if (count($articles_page) > 0): ?>
                            <!-- 文章列 -->
                            <div class="card ">
                ...
                <?php else: ?>
                  <!-- 根據過濾條件顯示不同的訊息 -->
                  <div class="alert alert-danger text-center text-white mt-3" role="alert">
                    <?= ($is_deleted === 1) ? '目前沒有下架文章' : '找不到符合的文章' ?>
                  </div>
                <?php endif; ?>
              <?php else: ?>
                <!-- 顯示所有文章的原始表格 -->
                      ...
                      <table class="table align-items-center mb-0">
                        <thead class="bg-gradient-dark">
                          <tr>
                            ...
                            <th class="text-uppercase text-secondary text-center text-sm text-white" style="width:10%">編輯者</th>
                            <th class="text-uppercase text-sm text-white" style="width:35%" colspan="2">內文</th>
                            <th class="text-uppercase text-sm text-center text-white" style="width:10%">最後更新時間</th>
                            <th class="text-center text-uppercase text-sm text-white" style="width:5%">檢視</th>
                            <th class="text-center text-uppercase text-sm text-white" style="width:5%">編輯</th>
                            <th class="text-center text-uppercase text-sm text-white" style="width:5%">狀態</th>
                          </tr>
                        </thead>
                        <tbody>
                          <?php foreach ($articles_page as $article): ?>
                            <tr>
                              ...
                              <!-- 編輯 -->
                              <td class="align-middle text-center">
                                <a href="articleEdit.php?id=<?= $article['id'] ?>" class="text-danger font-weight-bold text-sm">
                                  <i class="fa-regular fa-pen-to-square"></i>
                                </a>
                              </td>
                              <!-- 文章狀態 -->
                              <td class="text-center align-middle">
                                <div class="form-check form-switch d-flex justify-content-center align-items-center">
                                  <input class="form-check-input toggle-delete" type="checkbox" role="switch"
                                    id="toggleSwitch<?= $article['id'] ?>" data-id="<?= $article['id'] ?>"
                                    <?= $article['is_deleted'] ? 'checked' : '' ?>>
                                  <label class="form-check-label" for="toggleSwitch<?= $article['id'] ?>"></label>
                                </div>
                              </td>
